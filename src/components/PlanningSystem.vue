...
const planLevels = ref<PlanLevel[]>([
  ...
  { id: 'quarter', name: '季度计划', enabled: false, current: false },
  { id: 'month', name: '月计划', enabled: false, current: false },
  { id: 'week', name: '周计划', enabled: false, current: false },
])
...
const moduleStatus = ref<Record<string, boolean>>({})

// 保存模块
const saveModule = (moduleId: string) => {
  moduleStatus.value[moduleId] = true
}

// 检查是否可以完成计划
const canComplete = computed(() => {
  return otherModules.every(module => moduleStatus.value[module.id])
})
...
const completePlan = () => {
  ...
  // 解锁下一个层级
  const currentIndex = planLevels.value.findIndex(level => level.current)
  if (currentIndex < planLevels.value.length - 1) {
    planLevels.value[currentIndex + 1].enabled = true
  }
